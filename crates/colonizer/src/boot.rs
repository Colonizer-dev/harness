//! The colony boot sequence: everything a launch does between a session being admitted and the
//! agent daemon inside the microVM answering — issue fetch, worktree, stack detection, model
//! routing, the agentd config and boot script, mounts, secrets, mesh join — with its phase
//! timings. Session creation and queueing live in sessions.rs and queue.rs; this is the part that
//! makes a `Starting` colony live.
...
use crate::{
    ...
    sessions::{
        ...
        colony_image, findings_enabled, stack_choice,
    },
    stack,
...
async fn boot_inner(app: &Shared, id: &str, resume: bool) -> Result<()> {
    ...
    let session_json = session_config(id, &agent, &runner_env, &prompt);
...
/// The agentd config written to session.json: who the colony is, where it works, the port agentd
/// listens on inside the VM, the agent entry with its resolved env, and the opening prompt. Built
/// from the final `runner_env` only — see the write site above.
fn session_config(id: &str, agent: &AgentModule, runner_env: &Map<String, Value>, prompt: &str) -> Value {
    json!({
        "session_id": id,
        "workspace": "/workspace",
...
mod tests {
    ...
    /// session.json is assembled in one place: the colony's identity, its workspace, the port
    /// agentd listens on, the agent entry with its resolved env, and the opening prompt — and
    /// nothing else.
    #[test]
    fn session_config_carries_the_colony_its_agent_and_its_prompt() {
        let agent = AgentModule {
            id: "claude-code".into(),
            name: String::new(),
            description: String::new(),
            dir: PathBuf::from("/no/such/agent-dir"),
            entry: vec!["node".into(), "runner.mjs".into()],
            needs_claude: true,
            schema: Value::Null,
        };
        let mut env = Map::new();
        env.insert("COLONIZER_MODEL".into(), Value::String("opus".into()));
        let config = session_config("colony-7", &agent, &env, "fix the flaky test");
        assert_eq!(config["session_id"], "colony-7");
        assert_eq!(config["workspace"], "/workspace");
        assert_eq!(config["listen"], format!("0.0.0.0:{AGENTD_PORT}"));
        assert_eq!(config["initial_prompt"], "fix the flaky test");
        assert_eq!(config["agent"]["module"], "claude-code");
        // The dir is not on this machine, so vm_command() keeps the entry as written.
        assert_eq!(config["agent"]["command"], json!(["node", "runner.mjs"]));
        assert_eq!(config["agent"]["env"]["COLONIZER_MODEL"], "opus");
        assert_eq!(
            config.as_object().map(|o| o.len()),
            Some(5),
            "exactly the five keys agentd reads"
        );
    }
}
