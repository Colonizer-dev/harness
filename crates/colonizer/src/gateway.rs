...
async fn proxy(State(app): State<Shared>, Path((id, _)): Path<(String, String)>, method: Method, uri: Uri, headers: HeaderMap, body: Bytes) -> Response {
    ...
    if let Some(info) = translation {
        return openai_response(&app, &colony, &provider, upstream, guards, timeout, &info).await;
    }
...
async fn openai_response(app: &Shared, colony: &str, provider: &Provider, upstream: reqwest::Response, guards: Guards, timeout: Duration, info: &openai::RequestInfo) -> Response {
    let id = provider.id.as_str();
    let status = upstream.status();
    let retry_after = upstream.headers().get("retry-after").cloned();
    if status.is_success() && info.stream {
        let recorder = usage_recorder(app, colony, provider);
        let body = stream_body(openai::translate_stream(upstream.bytes_stream(), info.model.clone(), recorder), guards, timeout, true);
        let mut response = Response::new(Body::from_stream(body));
