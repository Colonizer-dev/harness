...
impl App {
    ...
    pub async fn mesh(&self) -> Result<Arc<Mesh>> {
        ...
        let port =
            |key: &str| u16::try_from(setting_u64(&modules.mesh, &schema, key)).unwrap_or_default();
        let ports = Ports {
            control: port("control_port"),
            udp: port("udp_port"),
            socks: port("socks_port"),
        };
        let created = Arc::new(Mesh::new(
            &assets,
            &self.cfg.data_dir,
...
pub async fn resolve_host_claude_bin(cfg: &Settings) -> Result<PathBuf> {
    find_claude_bin(cfg, false).await.context(
        "no native Claude Code binary found; install Claude Code or set COLONIZER_CLAUDE_BIN",
    )
}
...
async fn status(State(app): State<Shared>) -> Json<Value> {
    ...
    msb.arg("--version");
    let (user, msb_version, claude_bin) = tokio::join!(
        github::viewer(&app),
        exec(&mut msb),
    ...
    let mesh = if !modules.mesh_enabled() {
        ...
        .assets
        .as_deref()
        .is_some_and(mesh::binaries_present)
    {
        ...
        match app.mesh().await {
            ...
            Err(e) => {
                json!({"enabled": true, "provider": "headscale", "state": "error", "error": format!("{e:#}")})
            }
        }
    };
    ...
    let asset = |rel: &str| {
        app.cfg
            .assets
            .as_ref()
            .is_some_and(|a| a.join(rel).exists())
    };
    Json(json!({
        "github": match user {
...
async fn host_guard(State(app): State<Shared>, req: Request, next: Next) -> Response {
    ...
    let bind_host = app
        ...
        .bind
        .rsplit_once(':')
        .map_or(app.cfg.bind.as_str(), |(h, _)| h);
    ...
    if !allowed {
        return (
            ...
            "Host not allowed (set COLONIZER_ALLOWED_HOSTS)",
        )
            .into_response();
    }
...
async fn main() -> Result<()> {
    ...
    let api = Router::new()
        ...
        .route(
            "/api/sandbox/pull",
            post(sandbox::pull_configured).get(sandbox::pull_status),
        )
        ...
        .route(
            "/api/providers/{id}",
            put(providers::put).delete(providers::delete),
        )
