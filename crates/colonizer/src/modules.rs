    ...
    match kind {
        ...
        "sandbox" => vec![p(
            ...
            json!({"type": "object", "properties": {
                ...
                "discard_vm": {"type": "boolean", "title": "Discard the microVM when quota-parking", "default": true,
                    "description": "When a provider's quota runs out, cold-park the colony: snapshot uncommitted work, discard the microVM and release its slot, then boot a fresh microVM on the kept worktree when the quota resets. Off keeps the microVM and the colony's slot and continues the same agent in place when the quota resets."},
