...
/// Vendors with an Anthropic-compatible endpoint worth one click. `custom` is anything else.
const PRESETS: [&str; 5] = ["deepseek", "zai", "alibaba", "local", "custom"];
...
pub async fn put(State(app): State<Shared>, Path(id): Path<String>, Json(req): Json<PutProvider>) -> ApiResult<Value> {
    ...
    if !PRESETS.contains(&preset.as_str()) {
        return Err(bad("preset must be deepseek, zai, alibaba, local or custom"));
    }
