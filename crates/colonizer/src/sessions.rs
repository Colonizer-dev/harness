...
/// The port agentd listens on inside every colony's microVM.
...
/// `boot::resolve_stack` wraps around it — the way `queue::has_room` and `watchdog::decide` are written.
pub(crate) fn stack_choice(configured: &str, detected: Option<&crate::presets::Detected>) -> (String, Option<String>) {
