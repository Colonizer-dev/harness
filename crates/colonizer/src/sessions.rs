...
use crate::{
    ...
    util::{dir_size, format_disk_size, random_token, read_trimmed, short_id, truncate, valid_repo, write_private},
    watchdog::Activity,
...
/// The 409 `resume` answers with when a colony is not in a resumable state — shared with the re-check
/// under the admission lock, so a resume that loses the race reports what the first check would have.
const NOT_RESUMABLE: &str = "this colony can't be resumed: it has to be stopped and still have its worktree";
...
pub async fn resume(State(app): State<Shared>, Path(id): Path<String>) -> ApiResult<Session> {
    ...
    if !can_resume(s.status, s.cleaned_up, s.git_admin_dir.is_some()) {
        return Err(client_error(StatusCode::CONFLICT, NOT_RESUMABLE));
    }
    ...
    let claimed = with_slot(&app.sessions, &s.org, max_parallel, org_limit, |sessions, room| {
        ...
        if !can_resume(x.status, x.cleaned_up, x.git_admin_dir.is_some()) {
            return Err(NOT_RESUMABLE); // another resume won the race between the handler and the lock
        }
