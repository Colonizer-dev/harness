...
"queued, waiting for a free slot"; `starting` reads "booting for \<dur\>; \<last finished phase\>
done, next phase running \<dur\>" (clocked from the boot attempt, else the colony's birth; the
remaining time is the total minus the finished phases' recorded ms, bottoming out at `0s`), or
"booting for \<dur\>; first phase running" when no phase finished yet;
