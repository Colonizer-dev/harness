...
Autofix. The default pipeline is validate-and-file only. The `publish` module (module kind `github-pr`)
has two settings that open the rest, each overridable per colony by `autofix?`/`automerge?` on
`POST /api/sessions` (§4): `autofix` (default `false`) spawns a fix colony per filed finding, and
`automerge` (default `false`, requires `autofix` on — it is a no-op by itself) merges a fix colony's
pull request when its review passes. The ladder: validate-and-file only → autofix → autofix + merge.

Review independence. Merging waits on a review, and the review is deliberately independent: a fresh
session on the orchestrator model, a different session id from the author colony, with no shared
context — the reviewer never sees the fix colony's transcript or its files. Self-review (the review
session being the author's own) is rejected in code and covered by a Rust test. A failing review is
posted as a PR comment and the pull request stays open for a human; merging happens only on a pass,
and only when `automerge` is on.
