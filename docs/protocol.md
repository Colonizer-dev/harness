...
| `GET /api/repos/{owner}/{repo}/branches` | `{repo, default, branches: [{name, sha, date, author, message, default, protected, colony, ahead, behind, pr: {number, title, url, isDraft} \| null}]}`: the 100 most recently committed branches of the bare clone, ahead/behind counted against the default branch, protection and open pull requests from GitHub |
| `GET /api/repos/{owner}/{repo}/tree?ref=` | Every file path at a ref (a branch name or a commit sha; the default branch when absent): `{repo, ref, sha, paths, truncated}`, at most 20,000 paths |
| `GET /api/repos/{owner}/{repo}/blob?path=&ref=` | One file at a ref: `{path, ref, sha, size, binary, too_large, text}`. `text` is `null` for a binary file (a NUL in its first 8 KB) or one over 1 MB. The path must be repository-relative without `..` |
| `GET /api/repos/{owner}/{repo}/history?path=&ref=` | A file's 30 most recent commits at a ref: `{path, ref, commits: [{sha, author, date, message}]}` |
...
| `GET /api/chat` | Chat conversations (a direct conversation with a model, no colony), most recently updated first: `{chats: [{id, title, model, system?, max_tokens, workspace?, created_at, updated_at}]}`. Stored on the Mothership as `<data>/chats/<id>.json` + `<id>.jsonl` and sent nowhere but the chosen model |
| `GET /api/chat/models` | `{default, claude: {available, reason}, providers: [{id, name, models}]}`. `default` is the summaries' cheap model. A plain Claude model is available only with an Anthropic API key (`sk-ant-api…`) or an Anthropic model provider — never the Claude subscription login, which is for colonies |
| `POST /api/chat` | Creates a conversation: `{title?, model?, system?, max_tokens? (default 4096, ≤ 32000), workspace?}`; an empty model takes `default` |
| `GET`, `PATCH`, `DELETE /api/chat/{id}` | One conversation and its messages `{chat, messages: [{id, role, content, ts, model?, input_tokens, output_tokens, cost_usd?, stopped, error?}]}`; `PATCH` renames or changes model, system, max_tokens or workspace; `DELETE` removes both files |
| `POST /api/chat/{id}/messages` | `{content}` (≤ 100 KB) or `{regenerate: true}`, optional `context: {colony?, file?: {repo, path, ref?}}` (a colony's summary and recent activity, and/or a repository file read from the Mothership's clone like `…/blob`, up to 60 KB, join the system prompt). Answers `application/x-ndjson`, streamed: `{"type":"delta","text"}` lines, then `{"type":"done","message"}` or `{"type":"error","message","message_record"}`. Anthropic-wire providers stream; `openai`-wire providers answer in one delta. Closing the request stops the reply; what was written is stored with `stopped: true`. Tokens and (priced providers') cost go to the spend journal as a `usage` row under the conversation's workspace, or the `chat` pseudo-org |
