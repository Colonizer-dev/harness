...
| `GET`, `PATCH`, `DELETE /api/chat/{id}` | One conversation and its messages `{chat, messages: [{id, role, content, ts, model?, input_tokens, output_tokens, cost_usd?, stopped, error?, parent_id?, first_token_ms?, latency_ms?, attachments?: [{kind, label}], candidate, lane?}]}`; `PATCH` renames, pins, or changes model, system, persona, max_tokens, temperature (a negative one clears it) or workspace; `DELETE` removes both files |
...
| Chat attachments | Up to 8 per message, `{kind, …}`: `colony {id}` (summary, status, error and recent activity), `file {repo, path, ref?}` (read from the Mothership's clone like `…/blob`; repository-relative, no `..`; ≤ 60 KB used), `map {repo}` (every component of the stored architecture map), `map_component {repo, component}` (one component, its files and connections), `snippet {label?, text}` (≤ 60 KB), `image {media_type: png/jpeg/gif/webp, data: base64 ≤ 5 MB, name?}` (only to Anthropic-wire models; an `openai`-wire model is refused with 400), `colonies_today {org?}` (colonies that moved in the last 24 h), `merged_prs {org?, days? 1–31, default 7}`. Their text joins the system prompt (≤ 240 KB together); an image becomes an image block on the message. The message stores only `{kind, label}` per attachment |
...
| `GET /api/chat/{id}/export` | The conversation as `text/markdown`, a download |
