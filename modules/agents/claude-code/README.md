...
the Legion runner contract (`docs/protocol.md` §2): commands as JSON lines on stdin, events as JSON
...
| --- | --- | --- |
| `LEGION_CLAUDE_BIN` | `/opt/claude/bin/claude` | Native Claude Code binary |
| `LEGION_MODEL` | Claude Code default | Model alias or ID |
| `LEGION_EFFORT` | model default | `low`, `medium`, `high`, `xhigh` or `max` |
